...
function print_left_eldy_menu($db,$menu_array_before,$menu_array_after,&$tabMenu,&$menu,$noout=0,$forcemainmenu='',$forceleftmenu='',$moredata=null)
{
	...
	if ($mainmenu)
	{
		...
		if ($mainmenu == 'commercial')
		{
			...
			// ELITASOFT
			if (! empty($conf->elitasoft->enabled))
			{
				$langs->load("common@elitasoft");
				$newmenu->add("/elitasoft/dpu/card.php?action=create&amp", $langs->trans("NewDPU"), 0, 1);
				$newmenu->add("/elitasoft/dpu/list.php", $langs->trans("List"), 1, 1 );
			}
