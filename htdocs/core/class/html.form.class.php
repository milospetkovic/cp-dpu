    ...
    function select_produits_fournisseurs_list($socid,$selected='',$htmlname='productid',$filtertype='',$filtre='',$filterkey='',$statut=-1,$outputmode=0,$limit=100)
    {
        ...
        if ($result)
        {
            ...
            while ($i < $num)
            {
				...
				// elitasoft
				$idprod = $objp->rowid;
				$supplierprice = price($objp->fprice, 0, '', 0, 0);
				// end elitasoft
                ...
                $out.=$opt;
                // Elitasoft change
                array_push($outarray, array('key'=>$outkey, 'value'=>$outref, 'label'=>$outval, 'qty'=>$outqty, 'discount'=>$outdiscount, 'disabled'=>(empty($objp->idprodfournprice)?true:false), 'idprod'=>$idprod, 'price_ht'=>$supplierprice));
