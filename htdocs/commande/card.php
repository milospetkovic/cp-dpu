		...
		*/
		...
		if ($action == 'modif')
		{
			...
			else
			{
 			...
 			$text.='<br/>'.$langs->trans('ConfirmUnvalidateOrderWithInfo');
// 			$formquestion=array();
// 			if (! empty($conf->stock->enabled) && ! empty($conf->global->STOCK_CALCULATE_ON_VALIDATE_ORDER) && $qualified_for_stock_change)
// 			{
// 				$langs->load("stocks");
		...
		*/
		...
		if ($action != 'presend')
		{
			...
			*/
			...
			// Eliatsoft - we don't need for order
...
<script type="text/javascript">
	attach_inplace_edit("'.DOL_URL_ROOT.'");';

	// focus on inserting position ajax input
	if ($object->statut==Commande::STATUS_DRAFT) 
	{		
		print '
		$(document).ready(function() {
