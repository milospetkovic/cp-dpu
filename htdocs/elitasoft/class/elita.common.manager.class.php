...
{	
	 ...
	 * @param boolean $force_prop set property on object event not exists
	 * @return multitype:array |boolean	returns array of objects, or false on error
	 */
	static function queryList($sql, $name='', $force_prop = false) {
		...
		if($result) {
			...
			while($i<$num) {
				...
				if(!empty($name)) {
					$list[]=ElitaCommonManager::resultToObject($objp, $name, true, $force_prop);
				} else {
					$list[]=$objp;
				}
				$i++;
	 ...
	 * 
	 * @param unknown $obj	object to insert
	...
	static function querySingle($sql, $name='', $force_prop = false) {
		...
		if($result) {
			...
			if($num>0){
				$objp = $db->fetch_object($result);
				if(!empty($name)) {
					return ElitaCommonManager::resultToObject($objp, $name, true, $force_prop);
				} else {
					return $objp;
				}
			} else {
	 ...
	 * 
	 * @param unknown $obj	object which values need to be updated
	...
	static function fetch($tbl_name, $id, $name) {
		global $db;
		$sql="SELECT * FROM ".MAIN_DB_PREFIX.$tbl_name;
		$sql.=" WHERE rowid=".$db->escape($id);
