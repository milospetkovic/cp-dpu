 ...
 */
class modElitasoft extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->numero = 474252;		// TODO Go on page http://wiki.dolibarr.org/index.php/List_of_modules_id to reserve id number for your module
		
		// Key text used to identify module (for permissions, menus, etc...)
		$this->rights_class = 'elitasoft';
		...
		$this->familyinfo = array('myownfamily' => array('position' => '001', 'label' => $langs->trans("Elitasoft moduli")));
		...
		$this->description = "Elitasoft - modul za funkcije";
		...
		$this->module_parts['menus'] = 1;
		$this->module_parts['tpl'] = 1;
		$this->module_parts['css'] = array( '/elitasoft/css/elitasoft.css.php' );
		$this->module_parts['js'] = array( '/elitasoft/js/elitasoft.js' );
		$this->module_parts['hooks'] = array( 'ordercard', 'ordersuppliercard', 'dpucard' , 'common');
		...
		$this->config_page_url = array("admin.php@elitasoft");
		...
		$this->langfiles = array("common@elitasoft");
	...
	public function init($options='')
	{
		...
		// save latest svn revision
		$this->save_latest_rev();

		$this->_load_tables('/elitasoft/sql/');
	...
	public function remove($options = '')
	{
		...
		$this->removeClassesFile();

		return $this->_remove($sql, $options);
	}
	
	function createClassesFile() {
		...
		$files=dol_dir_list(DOL_DOCUMENT_ROOT, $types="all", $recursive=1, $filter=".class.php", $excludefilter="", $sortcriteria="", $sortorder=SORT_ASC, $mode=0, $nohook=true);
		...
		foreach($files as $f) {
			$path=$f['fullname'];
			$class=$this->get_class_name_from_file($f['fullname']);
			if(!empty($class)) {
				$lines[]="\t'$class'=>'$path',";
			}
	...
	function appendElbVersion() {
		$folder=DOL_DATA_ROOT."/elitasoft/temp";
		$file=$folder."/version.txt";
		return " - Elitasoft ".$this->version." rev. ".file_get_contents($file);
	}
	
	function save_latest_rev() {
		global $db;
		$cmd = "svn info ".DOL_DOCUMENT_ROOT;
		exec($cmd, $output, $retval);
		if($retval == 0) {
			foreach($output as $line) {
				if(strpos($line, "Revision:")===0) {
					$last_rev = trim(substr($line,strlen("Revision:")));
					$folder=DOL_DATA_ROOT."/elitasoft/temp";
					$file=$folder."/version.txt";
					file_put_contents($file, $last_rev);
					return;
				}
			}
		}
