...
?>
	...
	// Free line
	// Elitasoft - commented free entry
		...
		if (! empty($object->element) && $object->element == 'contrat' && empty($conf->global->CONTRACT_SUPPORT_PRODUCTS)) $filtertype='1';
 		...
 		if (empty($senderissupplier))
 		{
			...
			if ($object->element=='commande') 
			{
				$status=1;
				$ajaxoptions=array(
						'update' => array('price_ht'=>'price_ht')// html id tags that will be edited with which ajax json response key
				);
			} 
			...
			$form->select_produits(GETPOST('idprod'), 'idprod', $filtertype, $conf->product->limit_size, $buyer->price_level, $status, 2, '', 1, $ajaxoptions,$buyer->id);
		}
		else
		{
			// Elita - change
			$ajaxoptions=array(
					'update' => array('qty'=>'qty','remise_percent' => 'discount', 'price_ht'=>'price_ht', 'idprod' => 'idprod')	// html id tags that will be edited with which ajax json response key
			);
			
			//$form->select_produits_fournisseurs($object->socid, GETPOST('idprodfournprice'), 'idprodfournprice', '', '', $ajaxoptions, 1);
			echo '<input type="hidden" value="" id="idprod" name="idprod" />';
			$form->select_produits_fournisseurs('', GETPOST('idprodfournprice'), 'idprodfournprice', '', '', $ajaxoptions, 1);
			// end Elita change
		}
 		echo '</span>';
	}

	// Elitasoft - removed condition parameter
...
// 	}
	...
	echo '<select name="tva_tx">';
	echo '<option value="0">0%</option>';
	echo '<option value="10">10%</option>';
	echo '<option value="20">20%</option>';
	echo '</select>';	 
	//echo '<input type="text" name="tva_tx" id="tva_tx" value="0">'.vatrate(0, true);
